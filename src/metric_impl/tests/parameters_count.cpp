...
TEST(ParametersCount, SingleParameter) {
    ...
    CountParametersMetric metric;
    EXPECT_EQ(std::get<int>(metric.Calculate(func).value), 1);
}
...
TEST(ParametersCount, TwoParameters) {
    ...
    CountParametersMetric metric;
    EXPECT_EQ(std::get<int>(metric.Calculate(func).value), 2);
}
...
TEST(ParametersCount, ThreeParameters) {
    ...
    CountParametersMetric metric;
    EXPECT_EQ(std::get<int>(metric.Calculate(func).value), 3);
}
...
TEST(ParametersCount, ManyParameters) {
    ...
    CountParametersMetric metric;
    EXPECT_EQ(std::get<int>(metric.Calculate(func).value), 5);
}
